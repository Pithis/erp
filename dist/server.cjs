...
function authenticateToken(req, res, next) {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (!token && simulatedRole) {
      if (simulatedRole === "HR Staff") {
        req.user = { id: "hr-user-01", email: "hr@agency.com", name: "Sarah Jenkins", role: "HR_STAFF", contractType: "FULL_TIME" };
        return next();
      } else if (simulatedRole === "Operations Staff") {
        req.user = { id: "ops-user-01", email: "ops@agency.com", name: "Marcus Brody", role: "OPERATIONS_STAFF", contractType: "FULL_TIME" };
        return next();
      } else if (simulatedRole === "Creative/Technical Staff") {
        req.user = { id: "creative-user-01", email: "creative@agency.com", name: "Ray Jones", role: "CREATIVE_SPECIALIST", contractType: "FREELANCE" };
        return next();
      } else if (simulatedRole === "Stakeholder") {
        req.user = { id: "stakeholder-user-01", email: "stakeholder@agency.com", name: "Arthur Pendelton", role: "STAKEHOLDER", contractType: "FULL_TIME" };
        return next();
      }
    }
  }
